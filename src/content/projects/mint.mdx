...
The platform is fronted by a single NGINX reverse proxy that routes HTTP traffic to the Next.js web app and to each of the API services. The transactions service and the admin service also reach the fraud, KYC, and wallet services over gRPC for synchronous lookups before settling a payment or rendering an admin view.
